...
namespace z3nIO
{
    public class BackpackWallet : IWallet
    {
        ...
        public BackpackWallet(IZennoPosterProjectModel project, Instance instance, bool log = false, string key = null, string fileName = "Backpack0.10.94.crx")
        {
            ...
            _logger =  new Logger(_project,null, logLevel: (log) ? LogLevel.Info : LogLevel.Off);
        }
        #endregion
        ...
        public void Launch()
        {
            ...
            _logger.Send($"Launch: file={_fileName}, extId={_extId}");
            new ChromeExt(_project, _instance).Switch(_extId);
            if (new ChromeExt(_project, _instance).Install(_extId, _fileName))
                Import();
            else
                Unlock();
