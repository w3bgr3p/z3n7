...
namespace z3n7.Api
{
    public class SuperMails
    {
        ...
        private string apikey = "wdk_c53b9ab59c47d7a296100db5fb166a38";
        ...
        {
            var json = _project.GET($"{baseurl}/email/order?token={apikey}", log:true, useNetHttp:netHttp, thrw:true);
        ...
        public string GetMail(int deadline = 60)
        {
...

            {
                Thread.Sleep(5000);
                d.Check(deadline);
                var body = _project.GET($"{baseurl}/email/getmessage?token={apikey}&id={id}", log:true, useNetHttp:netHttp, thrw:true , parse:true);
