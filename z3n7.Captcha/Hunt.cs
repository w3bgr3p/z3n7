...
namespace z3n7.Captcha
{
    public class Hunt : IDisposable
    {
        ...
        private readonly InferenceSession _shapeModel;
        private readonly InferenceSession _ballModel;

        public Hunt(IZennoPosterProjectModel project,  Instance instance)
        {
            _project = project ?? throw new ArgumentNullException(nameof(project));
            _instance = instance ?? throw new ArgumentNullException(nameof(instance));
            _shapeModel = OpenModel(Path.Combine(project.ReadEnv("ONNX_FOLDER"),"hunt-shape.onnx"));
            try
            {
                _ballModel = OpenModel(Path.Combine(project.ReadEnv("ONNX_FOLDER"),"hunt-ball.onnx"));
            }
            catch
            {
                _shapeModel.Dispose();
        ...
        public IReadOnlyList<HuntPoint> SolveAndSubmitShapes(float confidence = 0.30f, int minimumDelayMs = 700, int maximumDelayMs = 1300)
        {
            ...
            int imageWidth;
            int imageHeight;
            using (var image = DecodeBitmap(imageBase64))
            {
                imageWidth = image.Width;
                imageHeight = image.Height;
            }
            ...
            int displayWidth;
            int displayHeight;
            using (var displayedCanvas = DecodeBitmap(canvas.DrawToBitmap(true)))
            {
                displayWidth = displayedCanvas.Width;
                displayHeight = displayedCanvas.Height;
            }

            var points = FindShapePoints(imageBase64, confidence);
        ...
        public HuntBallResult DetectBall(string imageBase64, float confidence = 0.30f)
        {
            var detections = Detect(_ballModel, imageBase64, BallNames, confidence);
        ...
        private IReadOnlyList<HuntPoint> FindShapePoints(string imageBase64, float confidence)
        {
            var detections = Detect(_shapeModel, imageBase64, ShapeNames, confidence);
            ...
            foreach (var prompt in prompts)
            {
                ...
                if (target == null)
                {
                    var diagnostics = Detect(_shapeModel, imageBase64, ShapeNames, 0.01f);
                    throw new InvalidOperationException(
                        $"Hunt shape target for class {targetClass} was not detected. " +
                        "Detected: " + FormatDetections(detections) +
                        Environment.NewLine +
                        "Detections at confidence 0.01: " + FormatDetections(diagnostics));
                }
        ...
        private static InferenceSession OpenModel(string path)
        {
        ...
        private static IReadOnlyList<HuntDetection> Detect(
        ...
        private static DenseTensor<float> CreateInput(Bitmap source, out Letterbox letterbox)
        {
            var scale = Math.Min(
            ...
            using (var graphics = Graphics.FromImage(resized))
            {
                ...
                graphics.DrawImage(source, new Rectangle(padX, padY, width, height));
        ...
        private static Bitmap DecodeBitmap(string imageBase64)
        {
            ...
            var raw = imageBase64.StartsWith("data:", StringComparison.OrdinalIgnoreCase) &&
                      comma >= 0
        ...
        private static string FormatDetections(IEnumerable<HuntDetection> detections)
        {
