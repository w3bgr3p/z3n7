...
namespace z3n7
{
    public class GeeTestV4
    {
        ...
        public static string DetectIconSubtype(IList<string> targetImages)
        {
            ...
            using (var bitmap = DecodeBitmap(targetImage))
                if (LooksLikeClockTip(bitmap))
                    clockTips++;
        ...
        private static bool LooksLikeClockTip(Bitmap source)
        {
            if (source.Width < 45 || source.Height < 45)
                return false;

            var x0 = source.Width * 47 / 100;
            var x1 = source.Width * 55 / 100;
            var upper = CountDarkPixels(source, x0, source.Height * 34 / 100, x1, source.Height * 50 / 100);
            var lower = CountDarkPixels(source, x0, source.Height * 50 / 100, x1, source.Height * 66 / 100);

            // The two dots of the digital time separator occupy fixed compact regions.
            return upper >= 4 && upper <= 12 && lower >= 3 && lower <= 10;
        }

        private static int CountDarkPixels(Bitmap source, int x0, int y0, int x1, int y1)
        {
            var count = 0;
            for (var y = y0; y < y1; y++)
            for (var x = x0; x < x1; x++)
            {
                ...
                var blended = (gray * color.A + 255 * (255 - color.A)) / 255;
                if (blended < 145)
                    count++;
            }
            return count;
